    ...
    <div className="p-8">
        ...
        <div className="flex gap-2">
          {(['tous', 'bas', 'critiques'] as const).map((f) => (
            <button key={f} onClick={() => setFiltre(f)}
              ...
              {f === 'tous' ? 'Tous' : f === 'bas' ? 'Stock bas' : 'Peremptions'}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6 mb-6">
        <div className="bg-white rounded-xl shadow p-4">
          <p className="text-sm text-gray-500">Total medicaments</p>
      ...
      <div className="grid grid-cols-3 gap-6">
              ...
              {stockFiltre.map((med) => (
                <tr key={med.id}
                  ...
                  <td className="px-4 py-3 text-center">
                    {med.stockBas && <span className="px-2 py-1 bg-red-100 text-red-600 rounded-full text-xs">Bas</span>}
                    {med.lotsCritiques > 0 && <span className="px-2 py-1 bg-yellow-100 text-yellow-600 rounded-full text-xs ml-1">Peremption</span>}
                    {!med.stockBas && med.lotsCritiques === 0 && <span className="px-2 py-1 bg-green-100 text-green-600 rounded-full text-xs">OK</span>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          {selected ? (
            <>
              <h2 className="font-semibold text-gray-700 mb-4">{selected.nom}</h2>
              <p className="text-sm text-gray-500 mb-4">Lots actifs</p>
                ...
                <ul className="space-y-3">
                  {selected.lots.map((lot) => (
                    <li key={lot.id} className="border rounded-lg p-3 text-sm">
                      <p className="font-medium">{lot.numeroLot || 'Sans numero'}</p>
                      <p className="text-gray-500">Expire: {formatDate(lot.datePeremption)}</p>
                      <p className="text-green-600 font-medium">{lot.quantite} unites</p>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <p className="text-gray-400 text-sm text-center mt-8">Cliquez sur un medicament pour voir ses lots</p>
          )}
        </div>
      </div>
    </div>
